...
def regression6(x):
    return F.where(
        x < -5.0,
        (x**2) * 0.1 + x - 10.0,
        F.where(x < 5.0, F.sin(x * 2) * x, F.cos(x * 2) * 5.0 - x),
    )


regression_model = Ly.Sequential(Ly.Linear(1, 100), Ly.ReLU(), Ly.Linear(100, 1))
...
def regression_model_test():
    ...
    for refname, ref in regressions_reference:
        ...
        for name, optimiser, loss in regression_loss:
            ...
            model = Model.copy_from(regression_model, optimiser, loss)
